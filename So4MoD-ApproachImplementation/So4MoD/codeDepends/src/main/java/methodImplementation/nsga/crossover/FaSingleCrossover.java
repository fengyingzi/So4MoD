...
public class FaSingleCrossover extends AbstractCrossover {

    public FaSingleCrossover(CrossoverParticipantCreator crossoverParticipantCreator, float crossoverProbability) {
    ...
    private void singlePointCrossover(List<Chromosome> children, Chromosome p1, Chromosome p2, int pointIndex) {
        ...
        if (!historyRecord.containsKey(c1)) {
            historyRecord.put(c1, ifChromosomeOverload(c1));
//                System.out.println("add child " + historyRecord.size());
            children.add(c1);
        }
        if (!historyRecord.containsKey(c2)) {
            historyRecord.put(c2, ifChromosomeOverload(c2));
//                System.out.println("add child " + historyRecord.size());
            children.add(c2);
        }
    }
